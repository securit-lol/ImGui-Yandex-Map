   ...
   ImGui::Begin("WayInfo", nullptr, ImGuiWindowFlags_NoResize |
    ...
    if (api::data::near_city_1.get())
        ...
        if (api::data::near_city_2.get())
            ...
            ImGui::Separator();
            if (api::data::near_city_1.get() && api::data::near_city_2.get() &&
              !api::data::way_data.search.from.title.empty() && !api::data::way_data.search.to.title.empty()) {
                std::lock_guard<std::mutex> lock(api::data::way_mtx);
            ImGui::Text("Откуда: %s (%s)", api::data::way_data.search.from.title.c_str(), api::data::way_data.search.from.code.c_str());
            ImGui::Text("Куда: %s (%s)", api::data::way_data.search.to.title.c_str(), api::data::way_data.search.to.code.c_str());
